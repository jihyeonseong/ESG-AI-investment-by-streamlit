...
def main(start_data, end_data):
    ...
    num_neighbors = st.sidebar.slider("포트폴리오를 위한 유사 기업 수를 선택하세요", 1, 20, value=8)
    ...
    elif graph_metric == '유사 기업 분포도' and len(neighbors)!=0:
    ...
    elif graph_metric == '유사도 점수 차트' and len(neighbors)!=0:
    ...
    if market_metric == 'USA':
        ...
        if portfolio_metric == 'Max Sharpe':
            ...
            pie_chart = alt.Chart(min_vol_allocation, title="마코위츠 포트폴리오 이론 예시입니다", padding={"left": 0, "top": 10, "right": 0, "bottom": 1}).mark_arc().encode(
                        theta=alt.Theta(field="allocation", type="quantitative"),
                        color=alt.Color(field="company", type="nominal"),
